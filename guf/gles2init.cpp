...
namespace {
	...
	"varying vec2 vTexCoord;\n"
	"void main() {\n"
		"gl_FragColor = vColor * texture2D(vTexCoord);\n"
	"}\n";
	;
...
void gufgles2::init(void)
{
	...
	makeProgram(programs[0],shaders[0],shaders[2],"vsLevel + fsGeom");
	makeProgram(programs[1],shaders[1],shaders[2],"vsObject + fsGeom");
}
