...
#define DOWNCAST(x,y) (x)(reinterpret_cast<unsigned long long int>(y))
    ...
    if(enet_host_service(serverhost, &event, timeout) > 0)
    {
        switch(event.type)
        {
            ...
            case ENET_EVENT_TYPE_RECEIVE:
                ...
                process(event.packet, DOWNCAST(int,event.peer->data)); 
                if(event.packet->referenceCount==0) enet_packet_destroy(event.packet);
                break;

            case ENET_EVENT_TYPE_DISCONNECT: 
                if(DOWNCAST(int,event.peer->data)<0) break;
                printf("disconnected client (%s)\n", clients[DOWNCAST(int,event.peer->data)].hostname);
                clients[DOWNCAST(int,event.peer->data)].type = ST_EMPTY;
                send2(true, -1, SV_CDIS, DOWNCAST(int,event.peer->data));
